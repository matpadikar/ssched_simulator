...
template <typename Iterator> bool unitRange(Iterator first, Iterator last) {
  ++first;
  return first == last;
}
...
Iterator radixSortPartition(Iterator first, Iterator last, Functor functor,
                            Value mask) {
...
template <typename Value, typename Iterator, typename Functor>
void radixIntroSort(Iterator first, Iterator last, Functor functor,
                    Value mask) {
...
template <typename Value> struct RadixSortSelector<Value, false> {
  template <typename Iterator, typename Functor>
  static void sort(Iterator first, Iterator last, Functor functor) {
...
template <typename Value> unsigned char valueByte(Value value, int byte) {
  return value >> (std::numeric_limits<unsigned char>::digits * byte);
}

template <typename Functor, typename Key>
void stableRadixIntroSort(Key *first, Key *last, Key *target, int byte,
                          Functor functor) {
...
template <typename Functor, typename Key>
void signedStableRadixIntroSort(Key *first, Key *last, Key *target, int byte,
                                Functor functor) {
  const int size = unsigned(std::numeric_limits<unsigned char>::max()) + 1;
  std::vector<int> counter(size);
  for (int i = 0; i < size; ++i) {
...
void stableRadixSignedSort(Iterator first, Iterator last, Functor functor) {
  ...
  try {
    ...
    for (int i = 0; i < int(sizeof(Value)) - 1; ++i) {
      if (dir) {
        stableRadixIntroSort(buffer, buffer + length, buffer + length, i,
                             functor);
      } else {
        stableRadixIntroSort(buffer + length, buffer + 2 * length, buffer, i,
                             functor);
      }
      dir = !dir;
    }
    if (dir) {
      ...
      signedStableRadixIntroSort(buffer + length, buffer + 2 * length, buffer,
                                 sizeof(Value) - 1, functor);
...
template <typename Value> struct StableRadixSortSelector<Value, false> {
  template <typename Iterator, typename Functor>
  static void sort(Iterator first, Iterator last, Functor functor) {
