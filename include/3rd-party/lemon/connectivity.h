...
template <typename Graph> bool connected(const Graph &graph) {
...
template <typename Graph> int countConnectedComponents(const Graph &graph) {
  checkConcept<concepts::Graph, Graph>();
...
public:
  ...
  StronglyConnectedCutArcsVisitor(const Digraph &digraph, ArcMap &cutMap,
                                  int &cutNum)
...
/// \see connected()
template <typename Digraph> bool stronglyConnected(const Digraph &digraph) {
...
public:
  ...
  CountBiNodeConnectedComponentsVisitor(const Digraph &graph, int &compNum)
      : _graph(graph), _compNum(compNum), _numMap(graph), _retMap(graph),
        _predMap(graph), _num(0) {}
...
public:
  ...
  BiNodeConnectedComponentsVisitor(const Digraph &graph, ArcMap &compMap,
                                   int &compNum)
      : _graph(graph), _compMap(compMap), _compNum(compNum), _numMap(graph),
        _retMap(graph), _predMap(graph), _num(0) {}
...
public:
  ...
  BiNodeConnectedCutNodesVisitor(const Digraph &graph, NodeMap &cutMap,
                                 int &cutNum)
      : _graph(graph), _cutMap(cutMap), _cutNum(cutNum), _numMap(graph),
        _retMap(graph), _predMap(graph), _num(0) {}
...
template <typename Graph> bool biNodeConnected(const Graph &graph) {
...
public:
  ...
  BiEdgeConnectedComponentsVisitor(const Digraph &graph, NodeMap &compMap,
                                   int &compNum)
      : _graph(graph), _compMap(compMap), _compNum(compNum), _numMap(graph),
        _retMap(graph), _predMap(graph), _num(0) {}
...
public:
  ...
  BiEdgeConnectedCutEdgesVisitor(const Digraph &graph, ArcMap &cutMap,
                                 int &cutNum)
      : _graph(graph), _cutMap(cutMap), _cutNum(cutNum), _numMap(graph),
        _retMap(graph), _predMap(graph), _num(0) {}
...
template <typename Graph> bool biEdgeConnected(const Graph &graph) {
...
/// \see acyclic()
template <typename Digraph> bool dag(const Digraph &digraph) {
...
void topologicalSort(const Digraph &digraph, NodeMap &order) {
  ...
  DfsVisit<Digraph, TopologicalSortVisitor<Digraph, NodeMap>> dfs(digraph,
                                                                  visitor);
...
/// \see dag()
template <typename Graph> bool acyclic(const Graph &graph) {
  checkConcept<concepts::Graph, Graph>();
  typedef typename Graph::Node Node;
  typedef typename Graph::NodeIt NodeIt;
  typedef typename Graph::Arc Arc;
...
template <typename Graph> bool tree(const Graph &graph) {
  checkConcept<concepts::Graph, Graph>();
  typedef typename Graph::Node Node;
  typedef typename Graph::NodeIt NodeIt;
  typedef typename Graph::Arc Arc;
...
public:
  ...
  BipartitePartitionsVisitor(const Digraph &graph, PartMap &part,
                             bool &bipartite)
...
template <typename Graph> bool bipartite(const Graph &graph) {
  using namespace _connectivity_bits;
...
bool bipartitePartitions(const Graph &graph, NodeMap &partMap) {
  ...
  BfsVisit<Graph, BipartitePartitionsVisitor<Graph, NodeMap>> bfs(graph,
                                                                  visitor);
  bfs.init();
  for (NodeIt it(graph); it != INVALID; ++it) {
    if (!bfs.reached(it)) {
...
template <typename Graph> bool loopFree(const Graph &graph) {
...
/// the given graph. It works for both directed and undirected graphs.
template <typename Graph> bool parallelFree(const Graph &graph) {
...
template <typename Graph> bool simpleGraph(const Graph &graph) {
  typename Graph::template NodeMap<int> reached(graph, 0);
  int cnt = 1;
  for (typename Graph::NodeIt n(graph); n != INVALID; ++n) {
    reached[n] = cnt;
