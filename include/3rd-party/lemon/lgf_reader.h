...
template <typename Value> struct DefaultConverter {
...
template <typename _Item> class MapStorageBase {
public:
  typedef _Item Item;
...
template <typename Value, typename Map1 = std::map<std::string, Value>,
...
template <typename GR> struct GraphArcLookUpConverter {
  const GR &_graph;
...
template <typename Functor> class LineSection : public Section {
private:
  Functor _functor;
...
template <typename Functor> class StreamSection : public Section {
private:
  Functor _functor;
...
template <typename DGR> class DigraphReader;
...
public:
  ...
  DigraphReader(DGR &digraph, std::istream &is = std::cin)
      : _is(&is), local_is(false), _digraph(digraph), _use_nodes(false),
        _use_arcs(false), _skip_nodes(false), _skip_arcs(false) {}
  ...
  DigraphReader(DGR &digraph, const std::string &fn)
      : _is(new std::ifstream(fn.c_str())), local_is(true), _filename(fn),
        _digraph(digraph), _use_nodes(false), _use_arcs(false),
...
public:
  ...
  /// label map.
  template <typename Map> DigraphReader &useNodes(const Map &map) {
  ...
  /// label map.
  template <typename Map> DigraphReader &useArcs(const Map &map) {
...
template <typename GR> class GraphReader;
...
public:
  ...
  GraphReader(GR &graph, std::istream &is = std::cin)
      : _is(&is), local_is(false), _graph(graph), _use_nodes(false),
        _use_edges(false), _skip_nodes(false), _skip_edges(false) {}
  ...
  GraphReader(GR &graph, const std::string &fn)
      : _is(new std::ifstream(fn.c_str())), local_is(true), _filename(fn),
        _graph(graph), _use_nodes(false), _use_edges(false), _skip_nodes(false),
        _skip_edges(false) {
    if (!(*_is)) {
      delete _is;
      throw IoError("Cannot open file", fn);
...
public:
  ...
  /// Use previously constructed node set, and specify the node
  /// label map.
  template <typename Map> GraphReader &useNodes(const Map &map) {
    checkConcept<concepts::ReadMap<Node, typename Map::Value>, Map>();
    LEMON_ASSERT(!_use_nodes, "Multiple usage of useNodes() member");
    _use_nodes = true;
    _writer_bits::DefaultConverter<typename Map::Value> converter;
  ...
  /// label map.
  template <typename Map> GraphReader &useEdges(const Map &map) {
...
template <typename BGR> class BpGraphReader;
...
public:
  ...
  BpGraphReader &node(const std::string &caption, Node &node) {
    typedef _reader_bits::DoubleMapLookUpConverter<Node, RedNodeIndex,
                                                   BlueNodeIndex>
        Converter;
  ...
  /// Use previously constructed node set, and specify the node
  /// label map.
  template <typename Map> BpGraphReader &useNodes(const Map &map) {
    checkConcept<concepts::ReadMap<Node, typename Map::Value>, Map>();
    LEMON_ASSERT(!_use_nodes, "Multiple usage of useNodes() member");
    _use_nodes = true;
    _writer_bits::DefaultConverter<typename Map::Value> converter;
  ...
  /// Use previously constructed edge set, and specify the edge
  /// label map.
  template <typename Map> BpGraphReader &useEdges(const Map &map) {
    checkConcept<concepts::ReadMap<Edge, typename Map::Value>, Map>();
    LEMON_ASSERT(!_use_edges, "Multiple usage of useEdges() member");
    _use_edges = true;
    _writer_bits::DefaultConverter<typename Map::Value> converter;
