...
template <typename GR, typename CM> struct KarpMmcDefaultTraits<GR, CM, true> {
  typedef GR Digraph;
  typedef CM CostMap;
  typedef typename CostMap::Value Cost;
...
public:
  ...
  KarpMmc(const Digraph &digraph, const CostMap &cost)
      : _gr(digraph), _cost(cost), _comp(digraph), _out_arcs(digraph),
        _cycle_cost(0), _cycle_size(1), _cycle_node(INVALID), _cycle_path(NULL),
        _local_path(false), _data(digraph),
