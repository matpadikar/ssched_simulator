...
template <class GR, class CM> struct MinCostArborescenceDefaultTraits {
...
private:
  ...
  Arc prepare(Node node) {
    ...
    DualVariable var(_dual_node_list.size() - 1, _dual_node_list.size(),
                     minimum.value);
...
public:
  ...
  template <class T> struct SetArborescenceMapTraits : public Traits {
  ...
  MinCostArborescence(const Digraph &digraph, const CostMap &cost)
      : _digraph(&digraph), _cost(&cost), _pred(0), local_pred(false),
        _arborescence(0), local_arborescence(false), _arc_order(0),
        _node_order(0), _cost_arcs(0), _heap_cross_ref(0), _heap(0) {}
