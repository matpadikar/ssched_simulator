...
///
/// \sa ConstMap
template <typename K, typename V> class NullMap : public MapBase<K, V> {
public:
  ///\e
  typedef K Key;
  ///\e
...
/// \relates NullMap
template <typename K, typename V> NullMap<K, V> nullMap() {
  return NullMap<K, V>();
}
...
/// \sa NullMap
/// \sa IdentityMap
template <typename K, typename V> class ConstMap : public MapBase<K, V> {
private:
  V _value;
...
/// \relates ConstMap
template <typename K, typename V> inline ConstMap<K, V> constMap(const V &v) {
  return ConstMap<K, V>(v);
}

template <typename K, typename V> inline ConstMap<K, V> constMap() {
  return ConstMap<K, V>();
}

template <typename T, T v> struct Const {};
...
///
/// \sa ConstMap
template <typename T> class IdentityMap : public MapBase<T, T> {
public:
  ///\e
  typedef T Key;
  ///\e
...
/// \relates IdentityMap
template <typename T> inline IdentityMap<T> identityMap() {
...
/// function.
template <typename V> class RangeMap : public MapBase<int, V> {
  template <typename V1> friend class RangeMap;
...
class SparseMap : public MapBase<K, V> {
  template <typename K1, typename V1, typename C1> friend class SparseMap;
...
/// \relates SparseMap
template <typename K, typename V, typename Compare>
inline SparseMap<K, V, Compare> sparseMap(const std::map<K, V, Compare> &map,
                                          const V &value = V()) {
...
inline CombineMap<M1, M2, F, V> combineMap(const M1 &m1, const M2 &m2,
                                           const F &f) {
...
///
/// \sa MapToFunctor
template <typename F, typename K = typename F::argument_type,
          typename V = typename F::result_type>
...
template <typename M> inline MapToFunctor<M> mapToFunctor(const M &m) {
...
///
/// \relates NegMap
template <typename M> inline NegMap<M> negMap(const M &m) {
  return NegMap<M>(m);
}
...
///
/// \relates NegWriteMap
template <typename M> inline NegWriteMap<M> negWriteMap(M &m) {
...
///
/// \relates AbsMap
template <typename M> inline AbsMap<M> absMap(const M &m) {
  return AbsMap<M>(m);
}
...
/// \sa FalseMap
/// \sa ConstMap
template <typename K> class TrueMap : public MapBase<K, bool> {
public:
  ///\e
  typedef K Key;
  ///\e
...
/// \relates TrueMap
template <typename K> inline TrueMap<K> trueMap() { return TrueMap<K>(); }
...
/// \sa TrueMap
/// \sa ConstMap
template <typename K> class FalseMap : public MapBase<K, bool> {
public:
  ///\e
  typedef K Key;
  ///\e
...
/// \relates FalseMap
template <typename K> inline FalseMap<K> falseMap() { return FalseMap<K>(); }
...
///
/// \sa NotWriteMap
template <typename M> class NotMap : public MapBase<typename M::Key, bool> {
  const M &_m;
...
///
/// \relates NotMap
template <typename M> inline NotMap<M> notMap(const M &m) {
  return NotMap<M>(m);
}
...
///
/// \relates NotWriteMap
template <typename M> inline NotWriteMap<M> notWriteMap(M &m) {
...
template <typename _Iterator, typename Enable = void> struct IteratorTraits {
  typedef typename std::iterator_traits<_Iterator>::value_type Value;
};

template <typename _Iterator>
struct IteratorTraits<
    _Iterator, typename exists<typename _Iterator::container_type>::type> {
...
///
/// \see RangeIdMap
template <typename GR, typename K> class IdMap : public MapBase<K, int> {
public:
...
/// \relates IdMap
template <typename K, typename GR> inline IdMap<GR, K> idMap(const GR &graph) {
...
public:
  ...
  public:
    ...
    explicit FalseIt(const IterableBoolMap &map)
        : Parent(map._sep < int(map._array.size()) ? map._array.back()
                                                   : INVALID),
          _map(&map) {}
...
namespace _maps_bits {
template <typename Item> struct IterableIntMapNode {
...
namespace _maps_bits {
template <typename Item, typename Value> struct IterableValueMapNode {
...
/// \see TargetMap
template <typename GR> class SourceMap {
public:
...
/// \relates SourceMap
template <typename GR> inline SourceMap<GR> sourceMap(const GR &graph) {
...
/// \tparam GR The digraph type.
/// \see SourceMap
template <typename GR> class TargetMap {
public:
  /// The key type (the \c Arc type of the digraph).
  typedef typename GR::Arc Key;
  /// The value type (the \c Node type of the digraph).
...
/// \relates TargetMap
template <typename GR> inline TargetMap<GR> targetMap(const GR &graph) {
...
/// \tparam GR The graph type.
/// \see BackwardMap
template <typename GR> class ForwardMap {
public:
...
/// \relates ForwardMap
template <typename GR> inline ForwardMap<GR> forwardMap(const GR &graph) {
...
/// \tparam GR The graph type.
/// \see ForwardMap
template <typename GR> class BackwardMap {
public:
  /// The key type (the \c Edge type of the digraph).
  typedef typename GR::Edge Key;
  /// The value type (the \c Arc type of the digraph).
...
/// \relates BackwardMap
template <typename GR> inline BackwardMap<GR> backwardMap(const GR &graph) {
...
public:
  ...
  typedef typename ItemSetTraits<
      Digraph, typename Digraph::Arc>::ItemNotifier::ObserverBase Parent;
...
template <typename GR, typename POT> class PotentialDifferenceMap {
public:
  /// Key type
  typedef typename GR::Arc Key;
  /// Value type
...
template <typename GR, typename Map>
typename Map::Key mapFind(const GR &gr, const Map &map,
