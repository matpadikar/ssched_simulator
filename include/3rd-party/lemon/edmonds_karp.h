...
template <typename GR, typename CAP> struct EdmondsKarpDefaultTraits {
...
public:
  ...
  EdmondsKarp(const Digraph &digraph, const CapacityMap &capacity, Node source,
              Node target)
  ...
  /// outgoing flow.
  template <typename FlowMap> void init(const FlowMap &flowMap) {
    createStructures();
  ...
  /// feasible flow.
  template <typename FlowMap> bool checkedInit(const FlowMap &flowMap) {
    createStructures();
    for (ArcIt e(_graph); e != INVALID; ++e) {
      _flow->set(e, flowMap[e]);
    }
