...
#else
template <typename GR, typename CM = typename GR::template ArcMap<int>,
...
public:
  ...
  template <typename T> struct SetPathTraits : public Traits {
    typedef T Path;
  };
...
public:
  ...
  HowardMmc(const Digraph &digraph, const CostMap &cost)
      : _gr(digraph), _cost(cost), _best_found(false), _best_cost(0),
        _best_size(1), _cycle_path(NULL), _local_path(false), _policy(digraph),
        _reached(digraph), _level(digraph), _dist(digraph), _comp(digraph),
        _in_arcs(digraph), INF(std::numeric_limits<LargeCost>::has_infinity
