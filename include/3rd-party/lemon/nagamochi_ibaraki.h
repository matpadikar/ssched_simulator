...
template <typename GR, typename CM> struct NagamochiIbarakiDefaultTraits {
...
#else
template <typename GR, typename CM = typename GR::template EdgeMap<int>,
...
public:
  ...
  NagamochiIbaraki(const Graph &graph, const CapacityMap &capacity)
      : _graph(graph), _capacity(&capacity), _local_capacity(false), _nodes(0),
        _arcs(), _edges(), _min_cut(), _heap_cross_ref(0),
        _local_heap_cross_ref(false), _heap(0), _local_heap(false),
  ...
  NagamochiIbaraki(const Graph &graph)
      : _graph(graph), _capacity(0), _local_capacity(false), _nodes(0), _arcs(),
        _edges(), _min_cut(), _heap_cross_ref(0), _local_heap_cross_ref(false),
        _heap(0), _local_heap(false), _next_rep(0), _cut_map(0) {}
...
public:
  ...
  template <typename CutMap> Value minCutMap(CutMap &cutMap) const {
    for (typename Graph::NodeIt n(_graph); n != INVALID; ++n) {
