...
/// \tparam GR Graph type.
template <typename GR> struct MaxFractionalMatchingDefaultTraits {
...
public:
  ...
  template <typename T> struct SetMatchingMapTraits : public Traits {
  ...
  template <typename T> struct SetElevatorTraits : public Traits {
    typedef T Elevator;
  ...
  template <typename T> struct SetStandardElevatorTraits : public Traits {
    typedef T Elevator;
    static Elevator *createElevator(const Graph &graph, int max_level) {
      return new Elevator(graph, max_level);
    }
...
public:
  ...
  MaxFractionalMatching(const Graph &graph, bool allow_loops = true)
      : _graph(graph), _allow_loops(allow_loops), _local_matching(false),
        _matching(0), _local_level(false), _level(0), _indeg(0) {}
...
public:
  ...
  MaxWeightedFractionalMatching(const Graph &graph, const WeightMap &weight,
      ...
      : _graph(graph), _weight(weight), _matching(0), _node_potential(0),
        _node_num(0), _allow_loops(allow_loops), _status(0), _pred(0),
        _tree_set_index(0), _tree_set(0),

        _delta1_index(0), _delta1(0), _delta2_index(0), _delta2(0),
  ...
  void init() {
    ...
    for (EdgeIt e(_graph); e != INVALID; ++e) {
      ...
      if (left == right && !_allow_loops)
        continue;
      _delta3->push(e, ((*_node_potential)[left] + (*_node_potential)[right] -
                        dualScale * _weight[e]) /
                           2);
    }
  }
...
public:
  ...
  MaxWeightedPerfectFractionalMatching(const Graph &graph,
      ...
      : _graph(graph), _weight(weight), _matching(0), _node_potential(0),
        ...
        _delta2_index(0), _delta2(0), _delta3_index(0), _delta3(0),
