...
public:
...
#ifdef DOXYGEN
  template <class T> int addColSet(T &t) { return 0; }
#else
  template <class T>
  ...
  template <class T>
  typename enable_if<typename T::MapIt::Value::LpCol, int>::type
...
#ifdef DOXYGEN
  template <class T> int addRowSet(T &t) { return 0; }
#else
  template <class T>
  ...
  void row(Row r, const Constr &c) {
    row(r, c.lowerBounded() ? c.lowerBound() : -INF, c.expr(),
  ...
  Row addRow(const Constr &c) {
    ...
    c.expr().simplify();
    r._id =
        _addRowId(_addRow(c.lowerBounded() ? c.lowerBound() - *c.expr() : -INF,
  ...
  Value colLowerBound(Col c) const { return _getColLowerBound(cols(id(c))); }
  ...
  /// Value or -\ref INF.
#ifdef DOXYGEN
  template <class T> void colLowerBound(T &t, Value value) { return 0; }
#else
  template <class T>
  ...
  Value colUpperBound(Col c) const { return _getColUpperBound(cols(id(c))); }
  ...
  /// Value or \ref INF.
#ifdef DOXYGEN
  template <class T> void colUpperBound(T &t, Value value) { return 0; }
#else
  template <class T1>
  ...
  /// Value, -\ref INF or \ref INF.
#ifdef DOXYGEN
  template <class T> void colBounds(T &t, Value lower, Value upper) {
    return 0;
  }
#else
  template <class T2>
  ...
  Value rowLowerBound(Row r) const { return _getRowLowerBound(rows(id(r))); }
  ...
  Value rowUpperBound(Row r) const { return _getRowUpperBound(rows(id(r))); }
...
public:
  ...
  /// Sets the type of the given column to the given type.
  ///
  void colType(Col c, ColTypes col_type) { _setColType(cols(id(c)), col_type); }
