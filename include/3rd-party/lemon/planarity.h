...
template <typename Graph> struct PlanarityVisitor : DfsVisitor<Graph> {
  ...
  PlanarityVisitor(const Graph &graph, PredMap &pred_map, TreeMap &tree_map,
...
template <typename Graph, bool embedding = true> struct NodeDataNode {
  int prev, next;
  int visited;
...
template <typename Graph> struct NodeDataNode<Graph, false> {
  int prev, next;
  int visited;
};

template <typename Graph> struct ChildListNode {
  typedef typename Graph::Node Node;
  Node first;
  Node prev, next;
};

template <typename Graph> struct ArcListNode {
  typename Graph::Arc prev, next;
};

template <typename Graph> class PlanarityChecking {
private:
  TEMPLATE_GRAPH_TYPEDEFS(Graph);
...
public:
  ...
  bool run() {
    ...
    Visitor visitor(_graph, pred_map, tree_map, order_map, order_list,
    ...
    for (int i = order_list.size() - 1; i >= 0; --i) {
      ...
      for (OutArcIt e(_graph, node); e != INVALID; ++e) {
        ...
        if (order_map[source] < order_map[target] && !tree_map[e]) {
          ...
          walkUp(target, source, i, pred_map, low_map, order_map, order_list,
                 node_data, merge_roots);
        }
      }
      ...
      for (typename MergeRoots::Value::iterator it = merge_roots[node].begin();
        ...
        int rn = *it;
        walkDown(rn, i, node_data, order_list, child_lists, ancestor_map,
                 low_map, embed_arc, merge_roots);
      }
...
private:
  ...
  void walkUp(const Node &node, Node root, int rorder, const PredMap &pred_map,
              const LowMap &low_map, const OrderMap &order_map,
              const OrderList &order_list, NodeData &node_data,
  ...
  void walkDown(int rn, int rorder, NodeData &node_data, OrderList &order_list,
                ChildLists &child_lists, AncestorMap &ancestor_map,
                LowMap &low_map, EmbedArc &embed_arc, MergeRoots &merge_roots) {
      ...
      while (n != rn) {
        ...
        if (!merge_roots[node].empty()) {
          ...
          if (!external(xnode, rorder, child_lists, ancestor_map, low_map)) {
            rd = true;
          } else if (!external(ynode, rorder, child_lists, ancestor_map,
                               low_map)) {
            rd = false;
        ...
        } else if (!external(node, rorder, child_lists, ancestor_map,
                             low_map)) {
          int nn =
  ...
  bool external(const Node &node, int rorder, ChildLists &child_lists,
                AncestorMap &ancestor_map, LowMap &low_map) {
...
template <typename GR> bool checkPlanarity(const GR &graph) {
...
template <typename Graph> class PlanarEmbedding {
private:
  TEMPLATE_GRAPH_TYPEDEFS(Graph);
  ...
  bool run(bool kuratowski = true) {
    ...
    for (int i = order_list.size() - 1; i >= 0; --i) {
      ...
      for (OutArcIt e(_graph, node); e != INVALID; ++e) {
        ...
        if (order_map[source] < order_map[target] && tree_map[e]) {
          initFace(target, arc_lists, node_data, pred_map, order_map,
                   order_list);
        }
      }
      ...
      for (typename MergeRoots::Value::iterator it = merge_roots[node].begin();
           it != merge_roots[node].end(); ++it) {
        int rn = *it;
        walkDown(rn, i, node_data, arc_lists, flip_map, order_list, child_lists,
      ...
      for (OutArcIt e(_graph, node); e != INVALID; ++e) {
        ...
        if (order_map[source] < order_map[target] && !tree_map[e]) {
          if (embed_arc[target] != INVALID) {
            if (kuratowski) {
              isolateKuratowski(e, node_data, arc_lists, flip_map, order_map,
                                order_list, pred_map, child_lists, ancestor_map,
                                low_map, embed_arc, merge_roots);
            }
            return false;
          }
    ...
    for (int i = 0; i < int(order_list.size()); ++i) {
      ...
      storeEmbedding(order_list[i], node_data, order_map, pred_map, arc_lists,
                     flip_map);
    }
...
private:
  ...
  void walkDown(int rn, int rorder, NodeData &node_data, ArcLists &arc_lists,
                FlipMap &flip_map, OrderList &order_list,
                ChildLists &child_lists, AncestorMap &ancestor_map,
                LowMap &low_map, EmbedArc &embed_arc, MergeRoots &merge_roots) {
  ...
  void initFace(const Node &node, ArcLists &arc_lists, NodeData &node_data,
  ...
  void findInternalPath(std::vector<Arc> &ipath, Node wnode, Node root,
                        TypeMap &type_map, OrderMap &order_map,
                        NodeData &node_data, ArcLists &arc_lists) {
    std::vector<Arc> st;
  ...
  void findPilePath(std::vector<Arc> &ppath, Node root, TypeMap &type_map,
                    OrderMap &order_map, NodeData &node_data,
                    ArcLists &arc_lists) {
    std::vector<Arc> st;
  ...
  int markExternalPath(Node node, OrderMap &order_map, ChildLists &child_lists,
                       PredMap &pred_map, AncestorMap &ancestor_map,
                       LowMap &low_map) {
  ...
  void markPertinentPath(Node node, OrderMap &order_map, NodeData &node_data,
                         ArcLists &arc_lists, EmbedArc &embed_arc,
                         MergeRoots &merge_roots) {
  ...
  void markFacePath(Node ynode, Node xnode, OrderMap &order_map,
                    NodeData &node_data) {
  ...
  void isolateKuratowski(Arc arc, NodeData &node_data, ArcLists &arc_lists,
                         FlipMap &flip_map, OrderMap &order_map,
                         OrderList &order_list, PredMap &pred_map,
                         ChildLists &child_lists, AncestorMap &ancestor_map,
                         LowMap &low_map, EmbedArc &embed_arc,
                         MergeRoots &merge_roots) {
    ...
    // Minor-A
    {
      ...
      if (root != _graph.source(arc)) {
        orientComponent(root, rn, order_map, pred_map, node_data, arc_lists,
                        flip_map, type_map);
        markFacePath(root, root, order_map, node_data);
        int xlp = markExternalPath(xnode, order_map, child_lists, pred_map,
                                   ancestor_map, low_map);
        int ylp = markExternalPath(ynode, order_map, child_lists, pred_map,
                                   ancestor_map, low_map);
        markPredPath(root, order_list[xlp < ylp ? xlp : ylp], pred_map);
        Node lwnode =
            findPertinent(ynode, order_map, node_data, embed_arc, merge_roots);

        markPertinentPath(lwnode, order_map, node_data, arc_lists, embed_arc,
                          merge_roots);
    ...
    if (!merge_roots[wnode].empty()) {
      ...
      if (low_map[rep] < rorder) {
        ...
        Node lwnode, lznode;
        markCommonPath(wnode, rorder, lwnode, lznode, order_list, order_map,
                       node_data, arc_lists, embed_arc, merge_roots,
                       child_lists, ancestor_map, low_map);

        markPertinentPath(lwnode, order_map, node_data, arc_lists, embed_arc,
                          merge_roots);
        int zlp = markExternalPath(lznode, order_map, child_lists, pred_map,
                                   ancestor_map, low_map);
    ...
    findInternalPath(ipath, wnode, root, type_map, order_map, node_data,
                     arc_lists);
    ...
    // Minor-C
    {
      if (type_map[_graph.source(ipath.front())] == HIGHY) {
        ...
        markPertinentPath(wnode, order_map, node_data, arc_lists, embed_arc,
                          merge_roots);
    ...
    // Minor-E*
    {
      ...
      if (!external(wnode, rorder, child_lists, ancestor_map, low_map)) {
        Node znode = findExternal(pynode, rorder, order_map, child_lists,
                                  ancestor_map, low_map, node_data);
        ...
        if (type_map[znode] == LOWY) {
          ...
          int xlp = markExternalPath(xnode, order_map, child_lists, pred_map,
                                     ancestor_map, low_map);
          int zlp = markExternalPath(znode, order_map, child_lists, pred_map,
                                     ancestor_map, low_map);
      ...
      int ylp = markExternalPath(ynode, order_map, child_lists, pred_map,
                                 ancestor_map, low_map);
      int wlp = markExternalPath(wnode, order_map, child_lists, pred_map,
                                 ancestor_map, low_map);
...
///
/// \see PlanarEmbedding
template <typename Graph> class PlanarDrawing {
public:
  TEMPLATE_GRAPH_TYPEDEFS(Graph);
...
private:
  ...
  void drawing(const AuxGraph &graph, const AuxEmbeddingMap &next,
               PointMap &point_map) {
...
public:
  ...
  template <typename EmbeddingMap> void run(const EmbeddingMap &embedding) {
...
template <typename ColorMap> class KempeFilter {
public:
  typedef typename ColorMap::Key Key;
  typedef bool Value;

  KempeFilter(const ColorMap &color_map, const typename ColorMap::Value &first,
...
template <typename Graph> class PlanarColoring {
public:
  TEMPLATE_GRAPH_TYPEDEFS(Graph);
