...
template <typename GR> class GrossoLocatelliPullanMc {
public:
...
public:
  ...
  ///
  /// \pre run() must be called before using this function.
  template <typename CliqueMap> void cliqueMap(CliqueMap &map) const {
...
private:
  ...
  template <typename SelectionRuleImpl> TerminationCause start() {
    if (_n == 0)
      return SIZE_LIMIT;
    if (_n == 1) {
      _best_clique[0] = true;
