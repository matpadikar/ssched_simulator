...
template <typename GR, typename CAP> struct MaxCardinalitySearchDefaultTraits {
  /// The digraph type the algorithm runs on.
  typedef GR Digraph;

  template <typename CM> struct CapMapSelector {
  ...
  template <typename CM> struct CapMapSelector<ConstMap<CM, Const<int, 1>>> {
  ...
  template <typename CapacityMap> struct HeapSelector {
    template <typename Value, typename Ref> struct Selector {
  ...
  typedef typename HeapSelector<CapacityMap>::template Selector<
      Value, HeapCrossRef>::Heap Heap;
...
#else
template <typename GR, typename CAP = ConstMap<typename GR::Arc, Const<int, 1>>,
...
public:
  ...
  template <class T> struct DefCapacityMapTraits : public Traits {
  ...
  /// for the algorithm.
  template <class T>
  struct SetCapacityMap : public MaxCardinalitySearch<Digraph, CapacityMap,
                                                      DefCapacityMapTraits<T>> {
    typedef MaxCardinalitySearch<Digraph, CapacityMap, DefCapacityMapTraits<T>>
        Create;
  };

  template <class T> struct DefCardinalityMapTraits : public Traits {
  ...
  template <class T> struct DefProcessedMapTraits : public Traits {
  ...
  struct SetHeap : public MaxCardinalitySearch<Digraph, CapacityMap,
                                               DefHeapTraits<H, CR>> {
    typedef MaxCardinalitySearch<Digraph, CapacityMap, DefHeapTraits<H, CR>>
        Create;
  };

  template <class H, class CR> struct DefStandardHeapTraits : public Traits {
    typedef CR HeapCrossRef;
    typedef H Heap;
    static HeapCrossRef *createHeapCrossRef(const Digraph &digraph) {
      return new HeapCrossRef(digraph);
...
public:
  ...
  MaxCardinalitySearch(const Digraph &digraph, const CapacityMap &capacity)
      : _graph(&digraph), _capacity(&capacity), local_capacity(false),
        _cardinality(0), local_cardinality(false), _processed(0),
        local_processed(false), _heap_cross_ref(0), local_heap_cross_ref(false),
  ...
  MaxCardinalitySearch(const Digraph &digraph)
      : _graph(&digraph), _capacity(0), local_capacity(false), _cardinality(0),
        local_cardinality(false), _processed(0), local_processed(false),
        _heap_cross_ref(0), local_heap_cross_ref(false), _heap(0),
        local_heap(false) {}
...
public:
  ...
  template <typename NodeBoolMap> void start(const NodeBoolMap &nm) {
