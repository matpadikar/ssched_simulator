...
template <typename _Map> class MapLess {
public:
  typedef _Map Map;
...
template <typename _Graph, bool _dir, typename _Map> class GraphArcMapLess {
public:
  typedef _Map Map;
...
template <typename Value, typename Map1 = std::map<Value, std::string>,
...
template <typename Graph> struct GraphArcLookUpConverter {
  const Graph &_graph;
...
template <typename DGR> class DigraphWriter;
...
public:
  ...
  DigraphWriter(const DGR &digraph, std::ostream &os = std::cout)
      : _os(&os), local_os(false), _digraph(digraph), _skip_nodes(false),
        _skip_arcs(false) {}
  ...
  void writeArcs() {
    ...
    *_os << std::endl;
    ...
    for (int i = 0; i < static_cast<int>(arcs.size()); ++i) {
      Arc a = arcs[i];
      _writer_bits::writeToken(*_os,
                               _node_index.find(_digraph.source(a))->second);
      *_os << '\t';
      _writer_bits::writeToken(*_os,
                               _node_index.find(_digraph.target(a))->second);
      *_os << '\t';
      if (label == 0) {
        std::ostringstream os;
        os << _digraph.id(a);
...
template <typename GR> class GraphWriter;
...
public:
  ...
  GraphWriter(const GR &graph, std::ostream &os = std::cout)
      : _os(&os), local_os(false), _graph(graph), _skip_nodes(false),
        _skip_edges(false) {}
...
template <typename BGR> class BpGraphWriter;
...
public:
  ...
  BpGraphWriter &node(const std::string &caption, const Node &node) {
    typedef _writer_bits::DoubleMapLookUpConverter<Node, RedNodeIndex,
                                                   BlueNodeIndex>
        Converter;
  ...
  void writeEdges() {
    ...
    *_os << std::endl;
    ...
    for (int i = 0; i < static_cast<int>(edges.size()); ++i) {
      Edge e = edges[i];
      _writer_bits::writeToken(*_os,
                               _red_node_index.find(_graph.redNode(e))->second);
      *_os << '\t';
