...
public:
  ...
  void first(Edge &arc) const { arc._id = arcs.size() / 2 - 1; }
...
public:
  ...
  SmartBpGraphBase()
      : nodes(), arcs(), first_red(-1), first_blue(-1), max_red(-1),
        max_blue(-1) {}
  ...
  RedNode redNode(Edge e) const { return RedNode(arcs[2 * e._id].target); }
  ...
  void next(RedNode &node) const { node._id = nodes[node._id].partition_next; }

  void first(BlueNode &node) const { node._id = first_blue; }

  void next(BlueNode &node) const { node._id = nodes[node._id].partition_next; }
