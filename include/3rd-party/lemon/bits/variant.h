...
template <int left, int right> struct CTMax {
...
// and destruction.
template <typename _First, typename _Second> class BiVariant {
public:
...
template <int _idx, typename _TypeMap> struct Memory {
...
template <typename _TypeMap> struct Memory<-1, _TypeMap> {
...
template <int _idx, typename _TypeMap> struct Size {
  static const int value =
      CTMax<sizeof(typename _TypeMap::template Map<_idx>::Type),
            Size<_idx - 1, _TypeMap>::value>::value;
};

template <typename _TypeMap> struct Size<0, _TypeMap> {
...
// 0.75
//\endcode
template <int _num, typename _TypeMap> class Variant {
public:
  ...
  const typename TypeMap::template Map<_idx>::Type &get() const {
    ...
    return *reinterpret_cast<
        const typename TypeMap::template Map<_idx>::Type *>(data);
  }

  // \brief Gets the current value of the type with \c _idx index.
  //
  // Gets the current value of the type with \c _idx index.
  template <int _idx> typename _TypeMap::template Map<_idx>::Type &get() {
    LEMON_DEBUG(_idx == flag, "Variant wrong index");
...
template <int _index, typename _List> struct Get {
  typedef typename Get<_index - 1, typename _List::Next>::Type Type;
};

template <typename _List> struct Get<0, _List> {
  typedef typename _List::Type Type;
};

struct List {};

template <typename _Type, typename _List> struct Insert {
  typedef _List Next;
  typedef _Type Type;
};

template <int _idx, typename _T0, typename _T1, typename _T2, typename _T3,
          typename _T4, typename _T5, typename _T6, typename _T7, typename _T8,
          typename _T9>
struct Mapper {
  typedef List L10;
...
// \see Variant
template <typename _T0, typename _T1 = void, typename _T2 = void,
          typename _T3 = void, typename _T4 = void, typename _T5 = void,
          typename _T6 = void, typename _T7 = void, typename _T8 = void,
          typename _T9 = void>
struct VariantTypeMap {
  template <int _idx> struct Map {
    typedef typename _variant_bits::Mapper<_idx, _T0, _T1, _T2, _T3, _T4, _T5,
                                           _T6, _T7, _T8, _T9>::Type Type;
  };
};
