...
public:
  ...
  HaoOrlin(const Digraph &graph, const CapacityMap &capacity,
      ...
      : _graph(graph), _capacity(&capacity), _flow(0), _source(), _node_num(),
        _first(), _last(), _next(0), _prev(0), _active(0), _bucket(0),
        _dormant(), _sets(), _highest(), _excess(0), _source_set(0), _min_cut(),
        _min_cut_map(0), _tolerance(tolerance) {}
      ...
      no_more_push:
        ...
        if (excess != 0) {
          if ((*_next)[n] == INVALID) {
            ...
            new_set->splice(new_set->end(), _sets.back(), _sets.back().begin(),
                            ++_highest);
