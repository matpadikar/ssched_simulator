...
template <typename V> struct BellmanFordDefaultOperationTraits<V, false> {
  typedef V Value;
  static Value zero() { return static_cast<Value>(0); }
...
template <typename GR, typename LEN> struct BellmanFordDefaultTraits {
...
public:
  ...
  BellmanFord(const Digraph &g, const LengthMap &length)
      : _gr(&g), _length(&length), _pred(0), _local_pred(false), _dist(0),
        _local_dist(false), _mask(0) {}
...
/// \tparam GR The type of the digraph.
/// \tparam LEN The type of the length map.
template <typename GR, typename LEN> struct BellmanFordWizardDefaultTraits {
  /// The type of the digraph the algorithm runs on.
  typedef GR Digraph;
...
public:
  ...
  BellmanFordWizardBase(const GR &gr, const LEN &len)
      : _graph(reinterpret_cast<void *>(const_cast<GR *>(&gr))),
        _length(reinterpret_cast<void *>(const_cast<LEN *>(&len))), _pred(0),
...
/// \tparam TR The traits class that defines various types used by the
/// algorithm.
template <class TR> class BellmanFordWizard : public TR {
  typedef TR Base;
...
public:
  ...
  template <class T> BellmanFordWizard<SetPredMapBase<T>> predMap(const T &t) {
  ...
  /// by the algorithm.
  template <class T> BellmanFordWizard<SetDistMapBase<T>> distMap(const T &t) {
  ...
  /// the shortest path to the target node.
  template <class T> BellmanFordWizard<SetPathBase<T>> path(const T &t) {
