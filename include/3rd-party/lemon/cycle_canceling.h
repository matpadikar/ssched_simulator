...
public:
  ...
  ///
  /// \return <tt>(*this)</tt>
  template <typename LowerMap> CycleCanceling &lowerMap(const LowerMap &map) {
    _has_lower = true;
  ...
  ///
  /// \return <tt>(*this)</tt>
  template <typename UpperMap> CycleCanceling &upperMap(const UpperMap &map) {
    for (ArcIt a(_graph); a != INVALID; ++a) {
  ...
  ///
  /// \return <tt>(*this)</tt>
  template <typename CostMap> CycleCanceling &costMap(const CostMap &map) {
    for (ArcIt a(_graph); a != INVALID; ++a) {
...
private:
  ...
  void startMinMeanCycleCanceling() {
    ...
    typedef
        typename HowardMmc<StaticDigraph,
                           CostArcMap>::template SetPath<SPath>::Create HwMmc;
    typedef typename HartmannOrlinMmc<
        StaticDigraph, CostArcMap>::template SetPath<SPath>::Create HoMmc;
