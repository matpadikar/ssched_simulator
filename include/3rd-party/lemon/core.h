...
template <typename Graph, typename Item> inline int countItems(const Graph &g) {
...
template <typename Graph, typename Enable = void> struct CountNodesSelector {
...
struct CountNodesSelector<
    Graph, typename enable_if<typename Graph::NodeNumTag, void>::type> {
...
template <typename Graph> inline int countNodes(const Graph &g) {
  return _core_bits::CountNodesSelector<Graph>::count(g);
}

namespace _graph_utils_bits {

template <typename Graph, typename Enable = void> struct CountRedNodesSelector {
  static int count(const Graph &g) {
...
/// function to query the cardinality of the node set.
template <typename Graph> inline int countRedNodes(const Graph &g) {
...
/// \e NodeNumTag tag then this function calls directly the member
/// function to query the cardinality of the node set.
template <typename Graph> inline int countBlueNodes(const Graph &g) {
...
template <typename Graph, typename Enable = void> struct CountArcsSelector {
  static int count(const Graph &g) {
...
struct CountArcsSelector<
    Graph, typename enable_if<typename Graph::ArcNumTag, void>::type> {
...
template <typename Graph> inline int countArcs(const Graph &g) {
...
template <typename Graph, typename Enable = void> struct CountEdgesSelector {
  static int count(const Graph &g) {
...
struct CountEdgesSelector<
    Graph, typename enable_if<typename Graph::EdgeNumTag, void>::type> {
...
template <typename Graph> inline int countEdges(const Graph &g) {
...
template <typename Digraph, typename Item, typename RefMap> class MapCopyBase {
public:
...
template <typename Digraph, typename Item, typename RefMap, typename FromMap,
          typename ToMap>
...
template <typename Digraph, typename Enable = void> struct DigraphCopySelector {
  template <typename From, typename NodeRefMap, typename ArcRefMap>
  static void copy(const From &from, Digraph &to, NodeRefMap &nodeRefMap,
                   ArcRefMap &arcRefMap) {
    to.clear();
...
struct DigraphCopySelector<
    Digraph, typename enable_if<typename Digraph::BuildTag, void>::type> {
  template <typename From, typename NodeRefMap, typename ArcRefMap>
  static void copy(const From &from, Digraph &to, NodeRefMap &nodeRefMap,
                   ArcRefMap &arcRefMap) {
...
template <typename Graph, typename Enable = void> struct GraphCopySelector {
  template <typename From, typename NodeRefMap, typename EdgeRefMap>
  static void copy(const From &from, Graph &to, NodeRefMap &nodeRefMap,
...
struct GraphCopySelector<
    Graph, typename enable_if<typename Graph::BuildTag, void>::type> {
  template <typename From, typename NodeRefMap, typename EdgeRefMap>
  static void copy(const From &from, Graph &to, NodeRefMap &nodeRefMap,
                   EdgeRefMap &edgeRefMap) {
...
template <typename BpGraph, typename Enable = void> struct BpGraphCopySelector {
  template <typename From, typename RedNodeRefMap, typename BlueNodeRefMap,
            typename EdgeRefMap>
  static void copy(const From &from, BpGraph &to, RedNodeRefMap &redNodeRefMap,
                   BlueNodeRefMap &blueNodeRefMap, EdgeRefMap &edgeRefMap) {
    to.clear();
...
struct BpGraphCopySelector<
    BpGraph, typename enable_if<typename BpGraph::BuildTag, void>::type> {
  template <typename From, typename RedNodeRefMap, typename BlueNodeRefMap,
            typename EdgeRefMap>
  static void copy(const From &from, BpGraph &to, RedNodeRefMap &redNodeRefMap,
                   BlueNodeRefMap &blueNodeRefMap, EdgeRefMap &edgeRefMap) {
...
#ifdef DOXYGEN
template <typename GR> bool undirected(const GR &g) { return false; }
#else
...
///  cg.run();
///\endcode
template <typename From, typename To> class DigraphCopy {
private:
...
public:
  ...
  /// destination digraph.
  template <typename NodeRef> DigraphCopy &nodeRef(NodeRef &map) {
  ...
  DigraphCopy &nodeMap(const FromMap &map, ToMap &tmap) {
    _node_maps.push_back(
        new _core_bits::MapCopy<From, Node, NodeRefMap, FromMap, ToMap>(map,
                                                                        tmap));
    return *this;
  }
  ...
  /// destination digraph.
  template <typename ArcRef> DigraphCopy &arcRef(ArcRef &map) {
    _arc_maps.push_back(
  ...
  template <typename ArcCrossRef> DigraphCopy &arcCrossRef(ArcCrossRef &map) {
    _arc_maps.push_back(
  ...
  DigraphCopy &arcMap(const FromMap &map, ToMap &tmap) {
    _arc_maps.push_back(
        new _core_bits::MapCopy<From, Arc, ArcRefMap, FromMap, ToMap>(map,
                                                                      tmap));
    return *this;
  }
  ...
  void run() {
    ...
    _core_bits::DigraphCopySelector<To>::copy(_from, _to, nodeRefMap,
                                              arcRefMap);
    for (int i = 0; i < int(_node_maps.size()); ++i) {
...
///  cg.run();
///\endcode
template <typename From, typename To> class GraphCopy {
private:
  typedef typename From::Node Node;
  typedef typename From::NodeIt NodeIt;
  typedef typename From::Arc Arc;
...
public:
  ...
  /// destination graph.
  template <typename NodeRef> GraphCopy &nodeRef(NodeRef &map) {
    _node_maps.push_back(
        new _core_bits::RefCopy<From, Node, NodeRefMap, NodeRef>(map));
    return *this;
  }
  ...
  template <typename NodeCrossRef> GraphCopy &nodeCrossRef(NodeCrossRef &map) {
    _node_maps.push_back(
        new _core_bits::CrossRefCopy<From, Node, NodeRefMap, NodeCrossRef>(
            map));
    return *this;
  ...
  /// destination graph.
  template <typename ArcRef> GraphCopy &arcRef(ArcRef &map) {
    _arc_maps.push_back(
        new _core_bits::RefCopy<From, Arc, ArcRefMap, ArcRef>(map));
    return *this;
  }
  ...
  template <typename ArcCrossRef> GraphCopy &arcCrossRef(ArcCrossRef &map) {
    _arc_maps.push_back(
        new _core_bits::CrossRefCopy<From, Arc, ArcRefMap, ArcCrossRef>(map));
    return *this;
  }
  ...
  /// destination graph.
  template <typename EdgeRef> GraphCopy &edgeRef(EdgeRef &map) {
  ...
  template <typename EdgeCrossRef> GraphCopy &edgeCrossRef(EdgeCrossRef &map) {
    _edge_maps.push_back(
  ...
  GraphCopy &edgeMap(const FromMap &map, ToMap &tmap) {
    _edge_maps.push_back(
        new _core_bits::MapCopy<From, Edge, EdgeRefMap, FromMap, ToMap>(map,
                                                                        tmap));
    return *this;
  }
...
///  cg.run();
///\endcode
template <typename From, typename To> class BpGraphCopy {
private:
  typedef typename From::Node Node;
...
public:
  ...
  /// the source graph, while the value type is the Node type of the
  /// destination graph.
  template <typename NodeRef> BpGraphCopy &nodeRef(NodeRef &map) {
    _node_maps.push_back(
        new _core_bits::RefCopy<From, Node, NodeRefMap, NodeRef>(map));
    return *this;
  }
  ...
  template <typename RedRef> BpGraphCopy &redRef(RedRef &map) {
    _red_maps.push_back(
  ...
  /// source graph.
  template <typename RedCrossRef> BpGraphCopy &redCrossRef(RedCrossRef &map) {
    _red_maps.push_back(
  ...
  /// value type is the Node type of the destination graph.
  template <typename BlueRef> BpGraphCopy &blueRef(BlueRef &map) {
  ...
  BpGraphCopy &blueCrossRef(BlueCrossRef &map) {
    _blue_maps.push_back(
        new _core_bits::CrossRefCopy<From, BlueNode, BlueNodeRefMap,
                                     BlueCrossRef>(map));
    return *this;
  }
  ...
  /// the source graph, while the value type is the Arc type of the
  /// destination graph.
  template <typename ArcRef> BpGraphCopy &arcRef(ArcRef &map) {
    _arc_maps.push_back(
        new _core_bits::RefCopy<From, Arc, ArcRefMap, ArcRef>(map));
    return *this;
  }
  ...
  /// is the Arc type of the destination graph, while the value type is
  /// the Arc type of the source graph.
  template <typename ArcCrossRef> BpGraphCopy &arcCrossRef(ArcCrossRef &map) {
    _arc_maps.push_back(
        new _core_bits::CrossRefCopy<From, Arc, ArcRefMap, ArcCrossRef>(map));
    return *this;
  }
  ...
  /// the source graph, while the value type is the Edge type of the
  /// destination graph.
  template <typename EdgeRef> BpGraphCopy &edgeRef(EdgeRef &map) {
    _edge_maps.push_back(
        new _core_bits::RefCopy<From, Edge, EdgeRefMap, EdgeRef>(map));
    return *this;
  }
  ...
  void run() {
    ...
    _core_bits::BpGraphCopySelector<To>::copy(_from, _to, redNodeRefMap,
                                              blueNodeRefMap, edgeRefMap);
    for (int i = 0; i < int(_node_maps.size()); ++i) {
      _node_maps[i]->copy(_from, nodeRefMap);
    }
    for (int i = 0; i < int(_red_maps.size()); ++i) {
...
template <typename Graph, typename Enable = void> struct FindArcSelector {
...
struct FindArcSelector<
    Graph, typename enable_if<typename Graph::FindArcTag, void>::type> {
  typedef typename Graph::Node Node;
  typedef typename Graph::Arc Arc;
...
template <typename Graph>
inline typename Graph::Arc findArc(const Graph &g, typename Graph::Node u,
...
///\sa findArc()
///\sa ArcLookUp, AllArcLookUp, DynArcLookUp
template <typename GR> class ConArcIt : public GR::Arc {
...
template <typename Graph, typename Enable = void> struct FindEdgeSelector {
  typedef typename Graph::Node Node;
  typedef typename Graph::Edge Edge;
...
struct FindEdgeSelector<
    Graph, typename enable_if<typename Graph::FindEdgeTag, void>::type> {
  typedef typename Graph::Node Node;
  typedef typename Graph::Edge Edge;
...
///\sa ConEdgeIt
template <typename Graph>
inline typename Graph::Edge findEdge(const Graph &g, typename Graph::Node u,
                                     typename Graph::Node v,
...
///
///\sa findEdge()
template <typename GR> class ConEdgeIt : public GR::Edge {
...
class DynArcLookUp
    : protected ItemSetTraits<GR,
                              typename GR::Arc>::ItemNotifier::ObserverBase {
  typedef
...
///\sa DynArcLookUp
///\sa AllArcLookUp
template <class GR> class ArcLookUp {
  TEMPLATE_DIGRAPH_TYPEDEFS(GR);
...
///\sa DynArcLookUp
///\sa ArcLookUp
template <class GR> class AllArcLookUp : public ArcLookUp<GR> {
