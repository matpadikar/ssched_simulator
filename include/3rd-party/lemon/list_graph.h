...
public:
  ...
  ListDigraphBase()
      : nodes(), first_node(-1), first_free_node(-1), arcs(),
        first_free_arc(-1) {}
  ...
  void next(Node &node) const { node.id = nodes[node.id].next; }

  void first(Arc &arc) const {
    int n;
    for (n = first_node; n != -1 && nodes[n].first_out == -1;
         n = nodes[n].next) {
    }
  ...
  void next(Arc &arc) const {
    if (arcs[arc.id].next_out != -1) {
      ...
      for (n = nodes[arcs[arc.id].source].next;
           n != -1 && nodes[n].first_out == -1; n = nodes[n].next) {
      }
      arc.id = (n == -1) ? -1 : nodes[n].first_out;
    }
  }

  void firstOut(Arc &e, const Node &v) const { e.id = nodes[v.id].first_out; }
  void nextOut(Arc &e) const { e.id = arcs[e.id].next_out; }

  void firstIn(Arc &e, const Node &v) const { e.id = nodes[v.id].first_in; }
  void nextIn(Arc &e) const { e.id = arcs[e.id].next_in; }
...
protected:
  void changeTarget(Arc e, Node n) {
    ...
    if (arcs[e.id].prev_in != -1)
      ...
      nodes[arcs[e.id].target].first_in = arcs[e.id].next_in;
  ...
  void changeSource(Arc e, Node n) {
    ...
    if (arcs[e.id].prev_out != -1)
      ...
      nodes[arcs[e.id].source].first_out = arcs[e.id].next_out;
...
protected:
  void changeV(Edge e, Node n) {
    ...
    if (arcs[2 * e.id].prev_out != -1) {
      arcs[arcs[2 * e.id].prev_out].next_out = arcs[2 * e.id].next_out;
    } else {
      nodes[arcs[(2 * e.id) | 1].target].first_out = arcs[2 * e.id].next_out;
    }
  ...
  void changeU(Edge e, Node n) {
    ...
    if (arcs[(2 * e.id) | 1].prev_out != -1) {
      ...
      nodes[arcs[2 * e.id].target].first_out = arcs[(2 * e.id) | 1].next_out;
    }
...
public:
  ...
  ListBpGraphBase()
      : nodes(), first_node(-1), first_red(-1), first_blue(-1), max_red(-1),
        max_blue(-1), first_free_red(-1), first_free_blue(-1), arcs(),
        first_free_arc(-1) {}
  ...
  RedNode redNode(Edge e) const { return RedNode(arcs[2 * e.id].target); }
  ...
  void next(RedNode &node) const { node.id = nodes[node.id].partition_next; }

  void first(BlueNode &node) const { node.id = first_blue; }

  void next(BlueNode &node) const { node.id = nodes[node.id].partition_next; }
  ...
  RedNode addRedNode() {
    ...
    if (first_red != -1)
      nodes[first_red].partition_prev = n;
    first_red = n;
  ...
  BlueNode addBlueNode() {
    ...
    if (first_blue != -1)
      nodes[first_blue].partition_prev = n;
    first_blue = n;
    nodes[n].partition_prev = -1;
