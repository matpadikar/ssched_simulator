...
private:
  template <typename KT, typename VT> class StaticVectorMap {
  public:
    typedef KT Key;
    typedef VT Value;
...
public:
  ...
  template <typename T> struct SetLargeCostTraits : public Traits {
    typedef T LargeCost;
  };
...
public:
  ...
  ///
  /// \return <tt>(*this)</tt>
  template <typename LowerMap> CostScaling &lowerMap(const LowerMap &map) {
    _has_lower = true;
  ...
  ///
  /// \return <tt>(*this)</tt>
  template <typename UpperMap> CostScaling &upperMap(const UpperMap &map) {
    for (ArcIt a(_graph); a != INVALID; ++a) {
  ...
  ///
  /// \return <tt>(*this)</tt>
  template <typename CostMap> CostScaling &costMap(const CostMap &map) {
    for (ArcIt a(_graph); a != INVALID; ++a) {
  ...
  ///
  /// \return <tt>(*this)</tt>
  template <typename SupplyMap> CostScaling &supplyMap(const SupplyMap &map) {
  ...
  template <typename Number> Number totalCost() const {
    Number c = 0;
    for (ArcIt a(_graph); a != INVALID; ++a) {
      int i = _arc_idb[a];
...
private:
  ...
  void start(Method method) {
    ...
    if (!optimal) {
      ...
      typename BellmanFord<StaticDigraph, LargeCostArcMap>::Create bf(sgr,
                                                                      cost_map);
      bf.init(0);
      bf.start();
