...
#ifndef SLIDING_WINDOW_QPS_H
#define SLIDING_WINDOW_QPS_H
...
/** @brief QPS-1 for each time slot in sliding window and accept multiple proposals
 *
 ...
 *  an empty/available time slot starting with the largest input port and from the 
 *  current time slot wrapping to the start of the sliding window of fixed size. 
 *
 */
class Sliding_Window_QPS :public BatchScheduler {
     ...
     *
     */
		Sliding_Window_QPS(std::string name, int numInputs, int  numOutputs, int batchSize, std::mt19937::result_type seed, int maxAccepts, bool batching, bool debug_mode);
...
/** @brief Best fit Bipartite matching in each time with QPS as initial matching
 *
 *  For each time slot first QPS-1 is run where the output ports try to schedule 
 *  each input port that proposed to them starting with the largest first and earliest 
 *  available time slot. This matching for each output port in each time slot is improved
 *  by running maximum cardinality bipartite matching on the initial QPS matching.
 *
 */
class SW_QPS_Best_Fit: public Sliding_Window_QPS{
	/// Allow SchedulerFactory class to access protected variables and methods
	friend class SchedulerFactory;
     ...
     *
     */
		SW_QPS_Best_Fit(std::string name, int numInputs, int  numOutputs, int batchSize,
			std::mt19937::result_type seed, bool debug_mode):
			Sliding_Window_QPS(name, numInputs,  numOutputs, batchSize, seed, 0, false, debug_mode){}
      ...
      * @param timeSlot time slot in the batch for which this round of SW-QPS is run
      */
		void qps(const IQSwitch* iqSwitch, size_t timeSlot) override;

}; ///< SW_QPS_Best_Fit

} ///< namespace saber

#endif ///< SLIDING_WINDOW_QPS_H
