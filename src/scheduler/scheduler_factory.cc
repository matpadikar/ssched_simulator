...
#include "batch_scheduler/sliding_window_qps.h"
#include "batch_scheduler/sliding_window_rr.h"
...
bool batching =
  ...
  } else if (name == "sliding_window_rr") {
  int frame_size =
	  (conf.count("frame_size") ? conf["frame_size"].get<int>() : 20);
  int cutoff =
	  (conf.count("cutoff") ? conf["cutoff"].get<int>() : num_inputs+num_outputs);
bool batching =
  ...
  unsigned seed = (conf.count("seed")
	  ...
	  : static_cast<unsigned>(
		  sys_clock_t::now().time_since_epoch().count()));
      bool debug_mode = conf.count("debug") && conf["debug"].get<bool>();
  sched = new Sliding_Window_RR(name, num_inputs, num_outputs, frame_size, seed, cutoff, batching, debug_mode);
